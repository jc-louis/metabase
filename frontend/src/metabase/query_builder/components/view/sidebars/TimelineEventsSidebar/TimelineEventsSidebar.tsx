...
import {
  ...
  TimelineName,
  EventListContainer,
} from "./TimelineEventsSidebar.styled";
...
type TimelineEventsSidebarProps = {
  timelines: Timeline[];
  hiddenTimelineSet: number[];
  onShowTimeline: (timelineId: number) => void;
  onHideTimeline: (timelineId: number) => void;
  onClose: () => void;
};

function TimelineEventsSidebar({
  timelines,
  hiddenTimelineSet,
  onShowTimeline,
  onHideTimeline,
    ...
    <SidebarContent title={t`Events`} onClose={onClose}>
      <TimelineList>
        {timelines.map(timeline => {
          const isVisible = !hiddenTimelineSet.includes(timeline.id);
          const onToggle = () => {
            if (isVisible) {
...
export default TimelineEventsSidebar;
